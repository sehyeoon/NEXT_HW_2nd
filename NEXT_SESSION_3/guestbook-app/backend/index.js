...
// 방명록 항목을 이름이나 메시지로 검색
app.get("/api/guestbook/search", async (req, res) => {
  const { query } = req.query;
  try {
    // 이름이나 메시지에 검색어가 포함된 항목을 찾음
    const result = await pool.query(
      "SELECT id, name, message, created_at FROM guestbook WHERE name ILIKE $1 OR message ILIKE $1 ORDER BY id DESC",
      [`%${query}%`]
    );
    res.json(result.rows);
  } catch (err) {
